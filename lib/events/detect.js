...
module.exports = async function detectEvent(sock, update) {
  try {
    const jid = jidFromEvent(update.id)
    if (!jid) return
    ...
    const target = jidFromEvent(update.participants?.[0])
    const author = jidFromEvent(update.author)
    ...
    console.error('❌ Error en detectEvent:', err)
  }
}