...
async function logMessage(m, conn) {
  ...
  try {
    // -------------------
    // Info del remitente segura
    // -------------------
    const senderJid = m.sender || m.key.participant || m.key.remoteJid
    let number = 'Desconocido'
    try {
      if (senderJid) number = PhoneNumber('+' + senderJid.replace('@s.whatsapp.net','')).getNumber('international') || senderJid
    } catch (e) {
      number = senderJid || 'Desconocido'
    }

    const name = senderJid || 'Anónimo'
    ...
    if (m.mentionedJid?.length) {
      mentions = m.mentionedJid.map(jid => '@' + jid.split('@')[0]).join(', ')
    }

    let replyTo = ''
    if (m.message.extendedTextMessage?.contextInfo?.participant) {
      replyTo = `(responde a ${m.message.extendedTextMessage.contextInfo.participant})`
    }
