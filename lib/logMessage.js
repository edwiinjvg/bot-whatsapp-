...
async function logMessage(m, conn) {
  ...
  try {
    ...
    let number = senderJid || 'Desconocido'
    try {
      if (senderJid) {
        number = PhoneNumber('+' + senderJid.replace('@s.whatsapp.net', '')).getNumber('international') || senderJid
      }
    } catch (err) {
      number = senderJid || 'Desconocido'
    }
    const name = await conn.getName(senderJid) || 'Anónimo'
